  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto p-5 text-slate-100 space-y-5">
      ...
      <div className="flex gap-2">
        <label htmlFor="verified-qa-query-input" className="sr-only">
          Ask LexPilot about contract obligations or covenants
        </label>
        <input
          id="verified-qa-query-input"
          aria-label="Ask LexPilot about obligations, cross-clause survival, or liability caps"
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && handleSend()}
          placeholder="Ask LexPilot about obligations, cross-clause survival, or liability caps..."
          className="flex-1 bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus-visible:ring-2 focus-visible:ring-indigo-400 transition"
        />
        <button
          onClick={() => handleSend()}
          aria-label="Submit question to LexPilot"
          disabled={isLoading || !query.trim()}
          className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition disabled:opacity-50 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
          {isLoading ? (
            ...
            >
              ...
              <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
                <h3 className="text-xs font-bold text-slate-100 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-indigo-400" aria-hidden="true"></span>
                  {item.question}
                </h3>
                ...
                <div className="flex items-center gap-2">
                  {item.multi_hop && (
                    <span className="font-semibold text-[11px] px-2.5 py-0.5 rounded-full bg-cyan-950/60 text-cyan-300 border border-cyan-800/40 flex items-center gap-1">
                      <Network className="w-3.5 h-3.5 text-cyan-400" aria-hidden="true" />
                      Multi-Hop Reasoning
                    </span>
                  )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                  {item.citations?.map((cit, cIdx) => (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      aria-label={`View cited clause ${cit.title} (${cit.clause_id}) on page ${cit.page_number}`}
                      onClick={() => onSelectClause(cit.clause_id)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          onSelectClause(cit.clause_id);
                        }
                      }}
                      className="bg-slate-950/70 hover:bg-slate-800/70 p-3 rounded-xl border border-slate-800 hover:border-indigo-500/80 cursor-pointer transition group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                    >
                      ...
                      <div className="bg-slate-950/80 p-3 rounded-xl border border-indigo-900/40 space-y-2">
                        ...
                        <div className="flex items-center gap-2 overflow-x-auto py-1 text-[11px] font-mono">
                          ...
                          <button
                            ...
                            aria-label={`View seed clause ${item.graph_path[0]}`}
                            className="px-2.5 py-1 rounded-lg bg-indigo-950/80 hover:bg-indigo-900/80 border border-indigo-600/50 text-indigo-200 cursor-pointer transition shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                          >
                          ...
                          <button
                            ...
                            aria-label={`View target clause ${item.graph_path[1]}`}
                            className="px-2.5 py-1 rounded-lg bg-indigo-950/80 hover:bg-indigo-900/80 border border-indigo-600/50 text-indigo-200 cursor-pointer transition shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                          >
                        ...
                        <div className="space-y-1.5">
                          ...
                          {prov.supporting_clauses?.map((sc, scIdx) => (
                            <button
                              ...
                              aria-label={`View supporting clause ${sc.clause_id}: ${sc.title}`}
                              className="w-full text-left cursor-pointer bg-slate-900/60 hover:bg-slate-800/80 p-2 rounded-lg border border-slate-800/80 flex items-center justify-between text-[11px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
                            >
                              <div className="flex items-center gap-2">
